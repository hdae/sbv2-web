...
- **JP-Extra フロントエンド**: DeBERTa char トークナイザと word2ph による BERT 特徴のタイル化を内蔵。
  ...
  パッケージに閉じています
- **DeBERTa は自動取得**: 量子化 DeBERTa とトークナイザ資産一式を `getDeberta()` が Hugging Face
  から取得します（コミット SHA 固定・サイズ/SHA-256 検証・Cache API キャッシュ・self-heal。
  [@hdae/fetch-cache](https://jsr.io/@hdae/fetch-cache) 使用）
- **辞書は @hdae/yomi に委譲**: テキスト解析・アクセント G2P・辞書取得（Hugging Face から自動ダウン
  ロード・キャッシュ・整合性検証）を担い、モデル非依存に保っています
- **リクエスト毎の合成パラメータ**: 話速（`lengthScale`）等のスカラー上書き・文頭/文末の無音・
  AIVM マニフェスト / 話者メタデータの型付き取得など、サーバー実装（VOICEVOX 互換等）に必要な
  API を備えています

モデルファイルは同梱しませんが、ユーザーが用意するのは音響 `.aivmx`（または `.onnx` +
style_vectors）だけです。DeBERTa・トークナイザ・辞書は既定で Hugging Face から自動取得されます。
...
// 量子化 DeBERTa + トークナイザ一式（Hugging Face から SHA 固定で取得・検証・キャッシュ）。
...
const adapter = await Sbv2ModelAdapter.createFromAivmx({
  aivmxBytes, // ユーザーが用意するのはこれだけ
  bertOnnxBytes,
  tokenizer,
});

// テキスト → 波形 → WAV。話速などのスカラーや文頭/文末無音はリクエスト毎に指定できる。
const wave = await synthesizeText(
  "こんにちは、今日はいい天気ですね。",
  dict,
  tokenizer,
  adapter,
  { scalars: { lengthScale: 1.1 }, postSilenceSec: 0.1 },
);
...
```typescript
import { getDeberta, Sbv2NodeModelAdapter } from "@hdae/sbv2-web/node";
import { getDictionary } from "@hdae/yomi/browser"; // Deno でも動く（Cache API 対応）
...
[migration-0.2.md](docs/migration-0.2.md) と
[ADR-0003](docs/decisions/0003-synthinput-public-contract.md) /
[ADR-0004](docs/decisions/0004-release-lifecycle-contract.md) を参照してください。

```typescript
import { readAivmxManifest, readAivmxMetadata } from "@hdae/sbv2-web";

// AIVM マニフェスト（話者・スタイルのカタログ）。既定で数 MB のアイコン等を strip。
const manifest = readAivmxManifest(aivmxBytes);
manifest.speakers[0].styles; // [{ name: "ノーマル", localId: 0, ... }, ...]

// 一度パースしたメタデータはアダプタ生成に再利用できる（巨大 protobuf の再走査回避）。
const metadata = readAivmxMetadata(aivmxBytes);
metadata.hyperParameters?.spk2id; // 話者名 → speakerId
...
adapter.numSpeakers; // hyper_parameters 由来のアクセサ
```
...
で終了）です。DeBERTa・トークナイザ・辞書は既定で Hugging Face から自動取得（初回のみダウンロード、
以後キャッシュ）されるので、**`--aivmx` だけで動きます**。web（WASM / WebGPU）経路はブラウザ版
サンプルで確認します。
...
主なオプション: `--device cpu|dml|cuda|webgpu`（既定 `cpu`）・`--deberta`+`--tokenizer`（ローカル資産を
使うときのみ・両方セットで指定）・`--dict`・`--out`（単発）/ `--out-dir`（REPL）・`--style-id` /
`--style-weight` / `--speaker-id`。
...
動き、量子化 DeBERTa + トークナイザは `getDeberta()`・辞書は `@hdae/yomi` の `getDictionary()` で
Hugging Face から自動取得（キャッシュ）します。Advanced トグルで全資産の手動選択に切り替わります。
...
で、量子化モデルを `model.onnx`・トークナイザ資産・LICENSE/NOTICE 付きで公開しています。
`getDeberta()` はこのリポジトリを焼き込みコミット SHA（`DEBERTA_REVISION`）で取得します（`revision`
オプションで差し替え可能）。b256 は最小 RAM の選択肢です。BERT 特徴の忠実度と RAM のトレードオフは
[Benchmarks](docs/benchmark.md) の int4 b32 行を参照してください。
